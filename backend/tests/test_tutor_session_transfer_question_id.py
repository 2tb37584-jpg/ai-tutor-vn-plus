...
def _load_migration():
    spec = importlib.util.spec_from_file_location("m06_tutor_transfer_question_id", _MIGRATION_PATH)
...
def test_transfer_question_id_round_trips_nullable_and_non_null_values() -> None:
    ...
    with Session(engine) as session:
        ...
        loaded.transfer_question_id = "g8alg.factorization.001"
        session.commit()
        session.expire_all()

        reloaded = session.get(TutorSession, tutor_session.id)
        assert reloaded is not None
        assert reloaded.transfer_question_id == "g8alg.factorization.001"


def test_transfer_question_id_migration_adds_and_removes_only_nullable_column() -> None:
    engine = create_engine("sqlite:///:memory:")
    metadata = MetaData()
    tutor_sessions = metadata.tables.get("tutor_sessions")
    ...
    with engine.begin() as connection:
        ...
        assert set(columns) == {"id", "transfer_question_id"}
        assert columns["transfer_question_id"]["nullable"] is True
