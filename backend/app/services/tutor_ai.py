...
def _analysis_prompt_with_skill_contract() -> str:
    ...
    return (
        ...
        "subskills. `skills[0]` is the PRIMARY skill. Choose exactly one primary skill: the best "
        "controlled skill matching the learner's main requested operation or pedagogical objective. "
        "Put supporting or prerequisite skills after the primary only when materially relevant. "
        "When an explicit higher-level target skill has prerequisites that are merely steps, keep "
        "the higher-level target primary. When one specific controlled skill completely describes "
        "the explicit learner objective, choose it over a broader umbrella skill. A task that only "
        "collects like terms uses algebra.expression.combine_like_terms as primary, not "
        "algebra.expression.simplify. Use algebra.expression.simplify when the task coordinates "
        "multiple expression transformations or broader simplification is the objective. For rational-expression simplification with a domain "
        "restriction, put algebra.rational_expression.simplify first and "
        "algebra.rational_expression.domain after it only if relevant. For general expression "
        "simplification, put algebra.expression.simplify first; "
        "algebra.expression.distributive_property and algebra.expression.combine_like_terms may "
        "follow as supporting skills. When the explicit task is to simplify the whole non-rational "
        "algebraic expression, algebra.expression.simplify remains primary even when distribution "
        "is a required step. When the explicit task is to expand, distribute, or remove parentheses, "
        "algebra.expression.distributive_property may be primary. For an explicit basic-identity expansion, put "
        "algebra.identity.basic first; distributive property may support it but must not replace "
        "the identity objective as primary. When a named basic identity is explicitly requested to "
        "rewrite an expression as a product, algebra.identity.basic is primary and "
        "algebra.factorization may be secondary if relevant. Choose the smallest directly relevant "
        "skill set.\n"
